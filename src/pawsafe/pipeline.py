...
def run(cfg: dict, start=None, end=None, route_time=None):
    ...
    if cfg["time"]["start"]:
        weather = weather[weather.timestamp >= pd.Timestamp(cfg["time"]["start"])]
    if cfg["time"]["end"]:
        weather = weather[weather.timestamp <= pd.Timestamp(cfg["time"]["end"])]
    ...
    scored.to_csv(
        processed / "edge_time_features.csv", index=False, encoding="utf-8-sig"
    )
    latest = scored.sort_values("timestamp").groupby("edge_id").tail(1)
    heat_map = edges.merge(
        latest[["edge_id", "timestamp", "cluster", "heat_cost"]],
        on="edge_id",
        how="left",
    ).to_crs(4326)
    ...
    nodes = gpd.GeoDataFrame(
        [
            {"node_id": i, "geometry": gpd.points_from_xy([n[0]], [n[1]])[0]}
    ...
    report = {
        ...
        "trees": len(trees),
        "time_rows": len(scored),
        "weather_start": str(weather.timestamp.min()),
        "weather_end": str(weather.timestamp.max()),
    }
    ...
    if start and end:
        route_all_modes(
            edges, scored, start, end, route_time or scored.timestamp.max(), cfg, out
        )
    return report
