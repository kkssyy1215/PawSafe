...
def fit_clusters(features: pd.DataFrame, cfg: dict, output_dir: Path):
    ...
    for k in cfg["clustering"]["k_values"]:
        ...
        for kind in ("kmeans", "gmm"):
            model = (
                KMeans(k, n_init=20, random_state=cfg["clustering"]["random_state"])
                if kind == "kmeans"
            ...
            s = (
                silhouette_score(X[eval_idx], labels[eval_idx])
                if len(np.unique(labels[eval_idx])) > 1
                else -1
            )
            d = (
                davies_bouldin_score(X[eval_idx], labels[eval_idx])
                if len(np.unique(labels[eval_idx])) > 1
                else np.inf
            )
    ...
    metrics["selection_score"] = metrics["silhouette"].rank(pct=True) + (
        -metrics["davies_bouldin"]
    ).rank(pct=True)
    best = metrics.sort_values(["selection_score", "silhouette"], ascending=False).iloc[
        0
    ]
    ...
    weights = (
        pd.Series(cfg["clustering"]["heat_direction_weights"]).reindex(cols).fillna(0)
    )
    ...
    metrics.to_csv(
        output_dir / "cluster_metrics.csv", index=False, encoding="utf-8-sig"
    )
    profiles.to_csv(
        output_dir / "cluster_profiles.csv", index=False, encoding="utf-8-sig"
    )
    (output_dir / "model_selection.json").write_text(
        json.dumps(best.to_dict(), ensure_ascii=False, indent=2, default=float),
        encoding="utf-8",
    )
...
def validate_optional(result: pd.DataFrame, cfg: dict, output_dir: Path):
    ...
    if not path.exists():
        pd.DataFrame(columns=["edge_id", "timestamp", "surface_temperature_c"]).to_csv(
            path, index=False, encoding="utf-8-sig"
        )
        return None
    measured = read_csv_auto(path)
    if measured.empty or not {"edge_id", "timestamp", "surface_temperature_c"}.issubset(
        measured
    ):
        return None
    ...
    report = {
        ...
        "spearman_heat_cost_vs_surface_temp": float(
            joined["heat_cost"].corr(joined["surface_temperature_c"], method="spearman")
        ),
    }
