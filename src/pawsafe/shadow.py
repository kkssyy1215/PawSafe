...
def solar_position(
    timestamp: pd.Timestamp, lat: float, lon: float, tz_name: str
) -> tuple[float, float]:
...
def _shadow_vector(
    height: float, altitude: float, azimuth: float, cap: float
) -> tuple[float, float]:
    length = min(cap, height / max(math.tan(math.radians(altitude)), 0.08))
    return -length * math.sin(math.radians(azimuth)), -length * math.cos(
...
def shadow_union(
    ...
    for geom, h in zip(buildings.geometry, buildings["height_m"]):
        dx, dy = _shadow_vector(
            float(h), altitude, azimuth, cfg["geometry"]["max_building_shadow_m"]
        )
        shadows.append(_extruded_shadow(geom, dx, dy))
    for geom, h, crown in zip(
        trees.geometry, trees["height_m"], trees["crown_width_m"]
    ):
        disk = geom.buffer(float(crown) / 2)
        dx, dy = _shadow_vector(
            float(h), altitude, azimuth, cfg["geometry"]["max_tree_shadow_m"]
        )
        shadows.append(_extruded_shadow(disk, dx, dy))
    return unary_union(shadows) if shadows else None


def shade_ratio_by_edge(
    edges: gpd.GeoDataFrame, shadow, spacing_m: float
) -> np.ndarray:
...
def calculate_shadows(edges, buildings, trees, timestamps, cfg):
    ...
    for ts in pd.to_datetime(timestamps):
        ...
        union = (
            shadow_union(buildings, trees, altitude, azimuth, cfg)
            if altitude > 0
            else None
        )
        ratios = (
            shade_ratio_by_edge(edges, union, cfg["geometry"]["edge_sample_spacing_m"])
            if altitude > 0
