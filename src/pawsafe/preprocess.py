    ...
    out = pd.DataFrame(
        {
            ...
            "register_height_m": pd.to_numeric(df[height], errors="coerce")
            if height
            else np.nan,
            "register_floors": pd.to_numeric(df[floors], errors="coerce")
            if floors
            else np.nan,
        }
    )
...
def load_buildings(cfg: dict, boundary: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    ...
    gdf["height_m"] = measured.where(measured > 0, estimated).fillna(
        cfg["geometry"]["default_building_height_m"]
    )
...
def load_trees(cfg: dict, boundary: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    ...
    cols = cfg["columns"]
    lon = find_column(df, cols["tree_lon"])
    lat = find_column(df, cols["tree_lat"])
    ...
    out = gpd.GeoDataFrame(
        {
            "height_m": pd.to_numeric(df.loc[mask, height], errors="coerce")
            if height
            else cfg["geometry"]["default_tree_height_m"],
            "crown_width_m": pd.to_numeric(df.loc[mask, crown], errors="coerce")
            if crown
            else cfg["geometry"]["default_tree_crown_width_m"],
        },
        geometry=gpd.points_from_xy(x[mask], y[mask]),
        crs=4326,
    ).to_crs(cfg["project_crs"])
    out["height_m"] = out["height_m"].where(
        out["height_m"].between(1, 40), cfg["geometry"]["default_tree_height_m"]
    )
    out["crown_width_m"] = out["crown_width_m"].where(
        out["crown_width_m"].between(0.5, 30),
        cfg["geometry"]["default_tree_crown_width_m"],
    )
...
def load_weather(cfg: dict) -> pd.DataFrame:
    ...
    for new, key in mapping.items():
        c = find_column(
            ...
            cols[key],
            required=(
                new in {"timestamp", "air_temperature_c", "solar_radiation_mj_m2"}
            ),
        )
    ...
    out["wind_speed_ms"] = (
        out["wind_speed_ms"].interpolate(limit_direction="both").fillna(0)
    )
...
def attach_pavement(
    ...
    if not candidates.empty:
        ...
        edges["surface_code"] = edges["edge_id"].map(intersection_map).fillna("unknown")
    ...
    if not missing_edges.empty:
        ...
        nearest = gpd.sjoin_nearest(
            ...
            how="left",
            max_distance=cfg["geometry"]["pavement_join_max_distance_m"],
            distance_col="pavement_distance_m",
        )

        nearest = nearest.sort_values("pavement_distance_m").drop_duplicates("edge_id")

        nearest = nearest[
            nearest["surface_code"].notna()
            & ~nearest["surface_code"].isin(["unknown", "-", "", "nan", "None"])
        ]
        ...
        fill_mask = edges["surface_code"].eq("unknown") & nearest_values.notna()
    ...
    # 팀 검토본은 Edge별로 확정한 흡수율을 제공한다. 포장재 코드는 유지하고,
    # 흡수율만 보정표로 덮어써서 OSM/SWM의 재질 배정을 훼손하지 않는다.
    revised_path = Path(cfg["files"].get("surface_absorptivity_revised", ""))
    if revised_path.is_file():
        revised = pd.read_excel(revised_path)
        required_columns = {"edge_id", "surface_absorptivity"}
        if not required_columns.issubset(revised.columns):
            raise ValueError(
                "흡수율 보정표에 edge_id와 surface_absorptivity 컬럼이 필요합니다."
            )
        if revised["edge_id"].duplicated().any():
            raise ValueError("흡수율 보정표에 중복 edge_id가 있습니다.")
        revised_values = pd.to_numeric(
            revised.set_index("edge_id")["surface_absorptivity"],
            errors="coerce",
        )
        if revised_values.isna().any() or (~revised_values.between(0, 1)).any():
            raise ValueError("흡수율 보정표 값은 0과 1 사이의 숫자여야 합니다.")
        mapped = edges["edge_id"].map(revised_values)
        edges["surface_absorptivity"] = mapped.fillna(edges["surface_absorptivity"])
