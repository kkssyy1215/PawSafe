...
export function MapTextSummary({ origin, destination, shortest, pawsafe, segments, walkMode = 'cool', visuallyHidden = false }: { origin: Place; destination: Place; shortest?: RouteStats; pawsafe?: RouteStats; segments?: HeatSegment[]; walkMode?: WalkMode; visuallyHidden?: boolean }) {
  ...
  const routeText = shortest && pawsafe
    ? `일반 최단 경로 ${formatDistance(shortest.distance_m)}, ${recommendedLabel} ${formatDistance(pawsafe.distance_m)}.`
    : pawsafe
...
const styles = StyleSheet.create({
  ...
  text: { ...typography.caption, color: colors.mutedText },
});
