...
export function MockMap({ origin, destination, shortest, pawsafe, segments, selectedSegmentId, onSegmentPress }: RouteMapProps) {
  ...
  return (
    <View accessible accessibilityLabel={`MVP 지도 예시, ${origin.name}에서 ${destination.name}까지`} style={styles.map}>
      ...
      {pawsafe ? <><View style={[styles.route, styles.pawsafeOne]} /><View style={[styles.route, styles.pawsafeTwo]} /><View style={[styles.route, styles.pawsafeThree]} /></> : null}
...
const styles = StyleSheet.create({
  ...
  route: { position: 'absolute', height: 7, borderRadius: 4 }, shortestOne: { width: 180, top: 180, left: 32, backgroundColor: colors.orange, transform: [{ rotate: '-9deg' }] }, shortestTwo: { width: 126, top: 145, left: 185, backgroundColor: colors.orange, transform: [{ rotate: '-28deg' }] },
  pawsafeOne: { width: 105, top: 173, left: 35, backgroundColor: colors.greenStrong, transform: [{ rotate: '-30deg' }] }, pawsafeTwo: { width: 135, top: 118, left: 115, backgroundColor: colors.greenStrong, transform: [{ rotate: '-6deg' }] }, pawsafeThree: { width: 92, top: 101, left: 227, backgroundColor: colors.greenStrong, transform: [{ rotate: '-34deg' }] },
