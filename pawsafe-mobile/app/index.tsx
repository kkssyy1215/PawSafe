    ...
    <ScreenContainer>
      <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" automaticallyAdjustKeyboardInsets>
          <ScreenHeader brand title="우리 강아지에게 걷기 좋은 길을 찾아볼까요?" description="노면온도와 그늘 정보를 비교해 발바닥 부담이 적은 길을 찾아요." />
          <View style={styles.sectionCard}>
            ...
            {!form.origin ? <CurrentLocationButton onSelect={(place) => { setValidationError(null); dispatch({ type: 'SET_PLACE', field: 'origin', place }); }} /> : null}
...
const styles = StyleSheet.create({
  ...
  sectionCard: { gap: spacing.md, padding: spacing.lg, borderRadius: 20, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surface }, step: { ...typography.caption, color: colors.greenStrong, fontWeight: '700' },
