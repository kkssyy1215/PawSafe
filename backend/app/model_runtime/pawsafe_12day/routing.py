...
def route(graph, start_xy, end_xy, heat_by_edge: dict, heat_weight: float):
    ...
    for geom in geoms:
        ...
        if coords:
            ...
            if previous_point.distance(Point(current_coords[-1])) < previous_point.distance(
                Point(current_coords[0])
            ):
...
def route_all_modes(
    ...
    graph, _lookup = build_graph(edges, cfg)
    ...
    matched_time = available_times.iloc[np.argmin(abs(available_times - requested_time))]

    snapshot = edge_time[pd.to_datetime(edge_time["timestamp"]) == matched_time]

    heat_by_edge = dict(zip(snapshot.edge_id, snapshot.heat_cost, strict=True))
        ...
        metric_rows.append(
            {
                ...
                "high_heat_ratio_pct": (
                    high_heat_distance / total_distance * 100 if total_distance > 0 else 0
                ),
                "low_heat_distance_m": low_heat_distance,
                "low_heat_ratio_pct": (
                    low_heat_distance / total_distance * 100 if total_distance > 0 else 0
                ),
