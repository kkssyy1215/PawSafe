...
def load_trees(cfg: dict, boundary: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    ...
    out["crown_width_m"] = out["crown_width_m"].where(
        out["crown_width_m"].between(0.5, 30), cfg["geometry"]["default_tree_crown_width_m"]
    )
...
def load_weather(cfg: dict) -> pd.DataFrame:
    ...
    for new, key in mapping.items():
        c = find_column(
            df,
            cols[key],
            required=(new in {"timestamp", "air_temperature_c", "solar_radiation_mj_m2"}),
        )
...
def attach_pavement(
    ...
    valid = (
        xmin.notna() & ymin.notna() & xmax.notna() & ymax.notna() & (xmax >= xmin) & (ymax >= ymin)
    )
    ...
    if not candidates.empty:
        ...
        candidates["overlap_length_m"] = candidates.apply(
            lambda row: row.geometry.intersection(pavement.geometry.loc[row["index_right"]]).length,
            axis=1,
        )
    ...
    edges["surface_absorptivity"] = (
        edges["surface_code"].map(cfg["heat"]["surface_absorptivity"]).fillna(unknown_absorptivity)
    )
