...
def shadow_union(
    buildings: gpd.GeoDataFrame, trees: gpd.GeoDataFrame, altitude: float, azimuth: float, cfg: dict
):
    if altitude <= 0:
        return None
    shadows = []
    for geom, h in zip(buildings.geometry, buildings["height_m"], strict=True):
...
def shade_ratio_by_edge(edges: gpd.GeoDataFrame, shadow, spacing_m: float) -> np.ndarray:
    ...
    for geom in edges.geometry:
        n = max(2, math.ceil(geom.length / spacing_m) + 1)
