...
이 문서는 온:길이 사용하는 외부 데이터의 출처와 표시·재배포 조건을 관리합니다.
...
- 최종 모델 적용: `backend/data/models/ongil_gmm_0815_1600/runtime/edge_cluster_heatcost.gpkg`
...
- 적용: 별도 기상 조회 API 기능. 최종 GMM 경로 요청에는 사용하지 않음
- 표시: `기상청 공공데이터(가공)`
...
- 표시: `기상청 공공데이터(가공)`
- 주의: 최종 모델은 `2026-08-15 16:00 KST` 분석에 사용된 가공 결과만
  런타임에 포함합니다. 경로 요청 시 ASOS를 다시 조회하지 않으며 원 관측값 대신
  온:길 상대 Heat Cost와 GMM 군집 확률을 제공합니다.
