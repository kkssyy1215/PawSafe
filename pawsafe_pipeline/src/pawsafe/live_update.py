...
def append_weather_csv(payload: dict[str, Any], path: Path) -> pd.DataFrame:
    ...
    current = pd.read_csv(path) if path.exists() else pd.DataFrame(columns=WEATHER_COLUMNS)
...
def append_asos_csv(payload: dict[str, Any], path: Path) -> pd.DataFrame:
    path.parent.mkdir(parents=True, exist_ok=True)
    row = {column: payload.get(column) for column in ASOS_COLUMNS if column != "fetched_at"}
...
def select_hour_template(features: pd.DataFrame, observed_at: pd.Timestamp) -> pd.DataFrame:
    ...
    available["distance"] = _hour_distance(available["timestamp"].dt.hour, observed_at.hour)
    template_time = available.sort_values(["distance", "timestamp"]).iloc[0]["timestamp"]
...
def build_live_features(
    ...
    previous = select_hour_template(historical_features, observed_at - pd.Timedelta(hours=1))
...
def score_with_saved_model(
    ...
    weights = pd.Series(cfg["clustering"]["heat_direction_weights"]).reindex(columns).fillna(0)
...
def write_live_outputs(
    ...
    merged = edges.merge(scored[columns], on="edge_id", how="left", suffixes=("_edge", ""))
...
def ensure_backend_graph_exports(root: Path, cfg: dict) -> dict[str, Path]:
    ...
    backend_exports = root.parent / "backend/data/exports"
    ...
    for index, (u, v, _key, data) in enumerate(graph.edges(keys=True, data=True)):
        ...
        forward = Point(coordinates[0]).distance(Point(u)) + Point(coordinates[-1]).distance(
            Point(v)
        )
        reverse = Point(coordinates[-1]).distance(Point(u)) + Point(coordinates[0]).distance(
            Point(v)
        )
        if reverse < forward:
            coordinates.reverse()
        ...
        edge_rows.append(
            {
                ...
                "display_name": str(edge_names.get(heat_edge_id, "이름 없는 보행 구간")),
                "bidirectional": True,
                "walkable": True,
...
def update_once(root: Path, api_url: str, asos_api_url: str | None = None) -> dict[str, Any]:
    ...
    backend_heat_path = root.parent / "backend/data/exports/edge_heat_cost.json"
