...
현재 앱은 산책 스타일에 따라 백엔드 공급자를 분리합니다.
...
cool
  → KMA AWS 서울 108번 최신 관측 조회
  → 12일 ASOS 기준자료와 최근 6시간 기상 창 생성
...
- 요청 시 입력: KMA AWS 최신 기온·습도·풍속·강수 관측
- 재계산: Edge 피처, 상대 Heat Cost, 경로
- 하지 않는 것: 요청마다 모델 재학습, 미래 단기예보, 노면온도(℃) 예측,
  절대 안전 판정

`KMA_AWS_AUTH_KEY`가 없는 환경에서는 자산 검증과 목 관측 통합 테스트까지는
